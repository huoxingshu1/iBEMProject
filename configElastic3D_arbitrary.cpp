...
/*
    Initilization of three arrays to record Gauss points
*/
void configElastic3D_arbitrary::Initialization_record()
{
    number_in_array = 500;
...
void configElastic3D_arbitrary::Readparticle_num()
{
...
void configElastic3D_arbitrary::Readpartical_info()
{
    ifstream myfile; myfile.open(particleInput);
    ...
    while (!myfile.eof()) {
        ...
        if (line[0] != '#' && line.length() != 0) {
            ...
            if (line_num < 2) {
                ...
                num_NE = stoi(line); line_num++;
                // assume 10-node quadratic tetrahedral elements
                eigen_connect = MatrixXi::Zero(num_NE, 10);

                // create arrays to store eigenstrain
                index_E_ij = new int* [Dimension * num_NN];

                // cout details about the particle
                cout << "number of 3D quadratic tetrahedral elements is" << " " << num_NE << endl;
            }

            // input the positions of nodes and elements
            else if (line_num < 4 + num_NN) {
                ss << line;
                ss >> eigen_point(line_num - 4, 0) >> eigen_point(line_num - 4, 1) >> eigen_point(line_num - 4, 2);
                line_num++; ss.clear();
            }
    ...
    for (int i = 0; i < Dimension * num_NN; i++) {
        index_E_ij[i] = new int[Dimension];
    }
    int id = 0;
...
void configElastic3D_arbitrary::ReadBEM_num()
{
    ifstream myfile;
    ...
    t_bc = MatrixXd(NE, 2 * 4 * Dimension);
}

void configElastic3D_arbitrary::ReadBEM_info()
{
    ...
    while (!myfile.eof()) {
        ...
        if (line[0] != '#' && line.length() != 0) {
            ...
            if (line_num < 5) {
                ...
                ss << line; ss >> NODES(line_num - 5, 0) >> NODES(line_num - 5, 1) >> NODES(line_num - 5, 2);
                line_num++; ss.clear();
            }

            else if (line_num < 5 + NN + NE) {
                // input the connection of the Elements, currently equal 3
                ss << line; ss >> NConnect(line_num - 5 - NN, 0) >> NConnect(line_num - 5 - NN, 1) >> NConnect(line_num - 5 - NN, 2) >> NConnect(line_num - 5 - NN, 3);
                line_num++; ss.clear();
            }
                ...
                ss << line;
                for (int ii = 0; ii < Dimension * 4 * 2; ii++) {
    ...
    HMAT = MatrixXd::Zero(Dimension * NN + 6 * num_NN, Dimension * NN + 6 * num_NN);
    GMAT = MatrixXd::Zero(Dimension * NN + 6 * num_NN, 4 * Dimension * NE);

    // Boundary solution vectors and eigenstrain in each particles
    U = VectorXd::Zero(Dimension * NN + 6 * num_NN);

    // tractions = number of nodes in one element * Dimension * Number of elements = 6 * NE
    T = VectorXd(Dimension * 4 * NE);

    // Define the Re-organized matrix
    A = MatrixXd::Zero(Dimension * NN + 6 * num_NN, Dimension * NN + 6 * num_NN);
    b = VectorXd::Zero(Dimension * NN + 6 * num_NN);

}

void configElastic3D_arbitrary::ReadPostProcess_info()
{
    ifstream myfile;
    ...
    while (!myfile.eof() && line_num <= nump) {
        ...
        if (line[0] != '#' && line.length() != 0) {
            if (line_num == 0) {
                ...
                ss << line; ss >> x_p(line_num - 1, 0) >> x_p(line_num - 1, 1) >> x_p(line_num - 1, 2);
                line_num++; ss.clear();
            }
        }
...
string configElastic3D_arbitrary::getbembuilder()
{
    return "BEMBuilderElastic3D_arbitrary";
}

string configElastic3D_arbitrary::getintegrator()
{
    return "integratorElastic3D_arbitrary";
}

string configElastic3D_arbitrary::getsystemsolver()
{
    return "systemSolverElastic3D_arbitrary";
}

string configElastic3D_arbitrary::getpostprocessor()
{
    return "postProcessorElastic3D_arbitrary";
}